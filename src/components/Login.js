...
const Login = () => {
    ...
    const onSubmitHandler = async (data) => {
        ...
        if (result?.payload?.success) {
            ...
            localStorage.setItem("token",result?.payload?.data?.token?.access_token)
                ...
                <form class="p-3 mt-3">
                    <div className='from-group'>
                        <div class="form-field d-flex align-items-center">
                            <User />
                            <input type="text" name="email" id="userName" {...register("email")} placeholder="Email" />
                        </div>
                    ...
                    <div className='from-group'>
                        <div class="form-field d-flex align-items-center">
                            <Key />
                            <input type="password" name="password" id="pwd" {...register("password")} placeholder="Password" />
                        </div>
                        <span className="text-danger">{errors?.password?.message}</span>
                    </div>
                    <button class="btn mt-3" onClick={handleSubmit(onSubmitHandler)}>Login</button>
                </form>
            </div>
        </React.Fragment>
    );
